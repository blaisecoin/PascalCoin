...
uses
  ...
  daemonapp,
  UOpenSSL, UCrypto, UNode, UFileStorage, UFolderHelper, UWalletKeys,
  UConst, ULog, UNetProtocol, UThread, URPC, UPoolMining, UAccounts;
...
begin
  ...
  FIniFile := TIniFile.Create(
      ExtractFileDir(Application.ExeName) + PathDelim + 'blaisecoin_daemon.ini');
  if FIniFile.ReadBool(CT_INI_SECTION_GLOBAL, CT_INI_IDENT_SAVELOGS, True) then
  begin
    _FLog.SaveTypes := CT_TLogTypes_ALL;
    _FLog.FileName := TFolderHelper.GetPascalCoinDataFolder + PathDelim +
        'blaisecoin_' + FormatDateTime('yyyymmddhhnn', Now) + '.log';
...
var
  ...
  Begin
    ...
    if FIniFile.ReadBool(CT_INI_SECTION_GLOBAL, CT_INI_IDENT_RPC_SAVELOGS, True) then
    begin
      ...
      TLog.NewLog(ltInfo, ClassName, 'Writing RPC log to file ' + FRPC.LogFileName);
    end else
    begin
      FIniFile.WriteBool(CT_INI_SECTION_GLOBAL, CT_INI_IDENT_RPC_SAVELOGS, False);
      TLog.NewLog(ltInfo, ClassName, 'Not writing RPC log to file');
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if (s = '') or (not TAccountComp.AccountKeyFromImport(s, pubkey, errors)) then
      begin
        if s <> '' then
...
//  if not SameText(sender, TPCDaemonThread.ClassName) then exit;
  if logtype in [lterror, ltinfo] then
  begin
    if AThreadID = MainThreadID then
      s := ' MAIN:'
    else
      s := ' TID:';
    WriteLn(formatDateTime('dd/mm/yyyy hh:nn:ss.zzz', Time) + s +
        IntToHex(AThreadID, 8) + ' [' + CT_LogType[Logtype] +
        '] <' + sender + '> ' + logtext);
  end;
end;
